...
import {browserLocalPersistence,getAuth,GoogleAuthProvider,onAuthStateChanged,setPersistence,signInWithPopup,signInWithCustomToken,signOut as webSignOut} from 'firebase/auth';
import {confirmSignOut} from './confirm.mjs';
const browserLoginId=new URLSearchParams(location.search).get('desktopLogin');
...
const appScripts=['/vendor/leaflet.js','/vendor/leaflet-rotate.js','/planner.js','/navigation.js','/simulation.js','/tracking.js','/reminders.js','/app.js','/settings-sync.js','/devices.js','/companion.js'];
...
async function bootstrap(){if(!authUser||appReady||booting)return;booting=true;authView('Opening your journeys…',false,true);try{const response=await fetch('/api/profile',{headers:{Accept:'application/json'}});if(!response.ok){if(response.status===403){authView('This Google account does not have access to Steady.',true,false,true);return;}throw Error('Your journeys could not be loaded.');}const data=await response.json();window.ROUTES=data.routes;window.TIMETABLE=data.timetable;window.STEADY_PROFILE=data.profile;window.STEADY_PERMISSIONS=data.permissions||{};try{const settingsResponse=await fetch('/api/settings',{cache:'no-store'});if(settingsResponse.ok)window.STEADY_SETTINGS=await settingsResponse.json();}catch{}for(const src of appScripts)await loadScript(src);appReady=true;authView();if(pendingUpdate)announceUpdate(pendingUpdate);if(native)registerPush().catch(()=>{});}catch(error){authView(error?.message||'Steady could not be opened. Please try again.',true,false,true);}finally{booting=false;}}
function setAuthUser(user){authUser=user||null;resolveAuth?.(authUser);resolveAuth=null;if(browserLoginId){if(authUser)showDesktopApproval();else authView('Sign in with Google to connect your desktop.');}else if(authUser)bootstrap();else authView();window.dispatchEvent(new CustomEvent('steady-auth-changed',{detail:{user:authUser}}));}
...
async function signIn(){
 if(window.SteadyDesktop&&!browserLoginId)return startBrowserSignIn();
 authView('Opening Google sign-in…',false,true);
 try{if(native){const result=await FirebaseAuthentication.signInWithGoogle({useCredentialManager:true});setAuthUser(result.user||(await FirebaseAuthentication.getCurrentUser()).user);}else{const provider=new GoogleAuthProvider();provider.setCustomParameters({prompt:'select_account'});await signInWithPopup(webAuth,provider);}if(native&&authUser)registerPush().catch(()=>{});}catch(error){authView(error?.message||'Google sign-in did not finish. Please try again.',true);}
}
let signingOut=false;
async function signOut(){if(signingOut)return;signingOut=true;try{if(!await confirmSignOut(document))return;if(native)await FirebaseAuthentication.signOut();else if(webAuth)await webSignOut(webAuth);setAuthUser(null);location.reload();}finally{signingOut=false;}}
...
window.addEventListener('DOMContentLoaded',()=>{document.getElementById('auth-signin')?.addEventListener('click',event=>{if(browserLoginId&&authUser)return approveDesktop();return event.currentTarget.dataset.switchAccount?signOut().catch(()=>authView('Could not switch accounts. Please try again.',true,false,true)):signIn();});document.getElementById('auth-different')?.addEventListener('click',signIn);document.addEventListener('click',event=>{if(event.target.closest('[data-auth-signout]'))signOut().catch(()=>authView('Could not sign out. Please try again.',true));});if(browserLoginId&&authUser)showDesktopApproval();else if(authUser)bootstrap();else authView(window.SteadyDesktop?'Sign in using your Google account in Chrome.':'Sign in to open Steady.');},{once:true});

function showDesktopApproval(){authView(`Use ${authUser?.email||'this Google account'} to sign in to Steady Companion?`,false,false,true);const button=document.getElementById('auth-signin'),different=document.getElementById('auth-different');if(button){button.textContent='Yes, use this account';button.dataset.switchAccount='';}if(different)different.hidden=false;}
async function approveDesktop(){const button=document.getElementById('auth-signin');button.disabled=true;try{const response=await fetch('/api/desktop-auth/approve',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:browserLoginId})}),data=await response.json();if(!response.ok)throw Error(data.error);authView('Approved. Return to Steady Companion; it will finish signing in automatically.',false,true);button.hidden=true;document.getElementById('auth-different').hidden=true;}catch(error){authView(error.message||'Sign-in could not be approved.',true,false,true);document.getElementById('auth-different').hidden=false;}}
async function startBrowserSignIn(){authView('Finish signing in in Chrome, then approve using the account for Steady.',false,true);try{const verifier=Array.from(crypto.getRandomValues(new Uint8Array(32)),byte=>byte.toString(16).padStart(2,'0')).join(''),challenge=Array.from(new Uint8Array(await crypto.subtle.digest('SHA-256',new TextEncoder().encode(verifier))),byte=>byte.toString(16).padStart(2,'0')).join(''),start=await fetch('/api/desktop-auth/start',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({challenge})}),request=await start.json();if(!start.ok)throw Error(request.error);await window.SteadyDesktop.openGoogleSignIn(request.id);const until=Date.now()+600000;while(Date.now()<until){await new Promise(resolve=>setTimeout(resolve,1500));const response=await fetch('/api/desktop-auth/poll',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:request.id,verifier})}),result=await response.json();if(!response.ok)throw Error(result.error);if(result.token){await signInWithCustomToken(webAuth,result.token);return;}}throw Error('Sign-in timed out. Please try again.');}catch(error){authView(error.message||'Browser sign-in did not finish. Please try again.',true,false);}}
